...
firstHalf [] = []
firstHalf [x] = [x]
firstHalf (x:xs) = x : firstHalf (init xs)
...
whileRight check x = eval $ check x
  where eval (Right nextX) = whileRight check nextX
        eval (Left end) = end
...
joinToLength n xs = filter ((==n) . length) [a ++ b| a <- xs, b <- xs]
...
sumRights = foldl sumEither 0
  where
    sumEither acc (Right x) = x + acc
    sumEither acc _ = acc
...
interpreter commands = getPrints $ foldl resolve initState commands
  where
    initState = ([], (0, 0))
    getPrints (path, _) = reverse path
    resolve (path, (x, y)) "up" = (path, (x, y + 1))
    resolve (path, (x, y)) "down" = (path, (x, y - 1))
    resolve (path, (x, y)) "right" = (path, (x + 1, y))
    resolve (path, (x, y)) "left" = (path, (x - 1, y))
    resolve (path, (x, y)) "printX" = (show x : path, (x, y))
    resolve (path, (x, y)) "printY" = (show y : path, (x, y))
